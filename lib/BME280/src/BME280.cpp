...
float BME280::CalculatePressure(
  ...
  uint16_t dig_P[9];
  for (int j = 0, i = 6; j < 9; j++, i += 2) {
    dig_P[j] = (m_dig[i + 1] << 8) | m_dig[i];
  }
  ...
  var1     = (((int64_t) dig_P[8]) * (pressure >> 13) * (pressure >> 13)) >> 25;
  var2     = (((int64_t) dig_P[7]) * pressure) >> 19;
  pressure = ((pressure + var1 + var2) >> 8) + (((int64_t) dig_P[6]) << 4);
  ...
  switch (unit) {
    ...
    case PresUnit_inHg: /* inHg */
      return final / 3386.3752577878f;  /* final pa * 1inHg/3386.3752577878Pa */
    case PresUnit_atm:  /* atm */
      return final / 101324.99766353f;  /* final pa * 1 atm/101324.99766353Pa */
    case PresUnit_bar:  /* bar */
      return final / 100000.0f;         /* final pa * 1 bar/100kPa */
    case PresUnit_torr: /* torr */
      return final / 133.32236534674f;  /* final pa * 1 torr/133.32236534674Pa */
    case PresUnit_psi:  /* psi */
      return final / 6894.744825494f;   /* final pa * 1psi/6894.744825494Pa */
    default:            /* Pa */
      return final;
  }
}
